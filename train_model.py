...
def train_model(
    ...
    new_acc = []
    if l != 1 and l != None and os.path.exists(f"models/seed-{seed}/models/{model_id}"):
        ...
        while True:
            ...
            for mini_batch_step, batch in it:
                ...
                if (mini_batch_step + 1) % k == 0:
                    ...
                    if current_iters in eval_points:
                        acc_test, _ = val_loop(
                            criterion=criterion,
                            dl_test=test_dl,
                            model=model,
    ...
    if save_model == True:
        log_message(
            message=f"Saving model {model_id} state dict", step=current_iters, seed=seed
        )
        Path(f"models/seed-{seed}/models").mkdir(parents=True, exist_ok=True)
        _path = f"models/seed-{seed}/models/{model_id}"
